...
@section('breadcrumb')
    ...
    <li>
        <a href="{{ route('superadmin::webs::edit') }}">{{ $web->name }}</a>
    </li>
@stop
...
@section('content')
    <div class="portlet light bordered form-fit">
        <div class="portlet-body form">
            <form action="{{ route('superadmin::webs::update') }}" method="POST" class="form-horizontal form-bordered form-label-stripped" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                ...
                <div class="form-group {{ $errors->has('subdomain') ? 'has-error' : '' }}">
                    ...
                    <div class="col-md-10">
                        <input type="text" value="{{ $web->subdomain }}" class="form-control" required>
